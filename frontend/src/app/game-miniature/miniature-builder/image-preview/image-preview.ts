...
export class ImagesPreviewComponent implements OnInit, OnChanges {
  ...
  miniatureImageUrl = computed(
    () => this.miniatureImage()?.image + '?timeStamp=$' + Date.now(),
  );
  ...
  onCanvasMouseMove(ev: MouseEvent) {
    ...
    if (this.isDragging) {
      ...
      (ev.currentTarget as HTMLCanvasElement).style.cursor = over
        ? 'grab'
        : 'default';
    }
  }

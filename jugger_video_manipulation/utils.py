...
def gen_proxy(
    ...
    Args:
        ...
        preset: rendering preset for the ffmpeg command
    Returns:
        out_file path of the proxy video
    """
    _ = preset
    if not (input_dir / "proxy").exists():
        (input_dir / "proxy").mkdir()
    out_file = input_dir / "proxy" / out_filename
    tmp_txt = Path("core/tmp") / Path("tmp_source.txt")
    with tmp_txt.open("w") as f:
        f.writelines(
            [
                "file '" + str((input_dir / "rushs" / v).absolute()) + "'\n"
                for v in video_files
            ]
        )
    ffmpeg_str = f"ffmpeg -hwaccel cuda -y  -f concat -safe 0 -i {tmp_txt} -preset fast -crf 32  -vf scale=640x360 -ar {16000} {out_file}"
...
def generate_preview(
    ...
    video_files: list[Path],
    out_filename: Path,
    tmp_dir: Path = Path("core/previews"),
) -> Path:
